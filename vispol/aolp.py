...
def LUT_matching(aop, lut, inverse=False):
    ...
    first, second = [0, 1] if not inverse else [1, 0]
    aop_matched = np.array([lut[second,np.argmin(np.abs(a - lut[first]))] if not np.isnan(a) else np.nan
...
def construct_matrix(shape, type="laplacian"):
    ...
    if type is "laplacian_diag":
        ...
        D = diags(A0, offsets=0, format='csc')
        U = diags([A1, Anx], offsets=[1, nx], format='csc')
        L = diags([A1, Anx], offsets=[-1, -1 * nx], format='csc')
...
def aop_colors(isoluminant=False):
    if isoluminant:
        ...
        c =   [ [ 0.7965425743657687, 0.40102683520413035, 0.4910817029503182, 1.0 ] ,
                ...
                [ 0.950139716668437, 0.7988733088110443, 0.48237219386703917, 1.0 ] ,
